        ...
        <div className='flex gap-2'>
          {
            props.tags.map((tag) => {
              ...
              return  (<Badge key={tag} color={color} >{tag}</Badge>)
              return  (<Badge key={tag} className={`bg-[${color}]`} >{tag}</Badge>)
            })
          }
        </div>
      </CardHeader>
        ...
        <div className='h-1/2 flex flex-col gap-3'>
          {
            filteredBlogs.map((blog) => {
              return <BlogEntry key={blog.title} title={blog.title} description={blog.description} tags={blog.tags} link={blog.link}/>
            })
          }
        </div>
      </ScrollArea>
